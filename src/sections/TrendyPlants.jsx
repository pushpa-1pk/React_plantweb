...
const TrendyPlants = ({ title, description, price, image, imagePosition = "left" }) => {
  return (
    <div
      className={`
        relative flex flex-col items-center text-center w-full bg-white/10 backdrop-blur-lg rounded-3xl text-white p-12 shadow-xl border border-white/50 gap-6
        md:flex-row md:text-left md:items-center md:h-[520px] md:rounded-[150px]
        ${imagePosition === "right" ? "md:flex-row-reverse" : ""}
      `}
    >
      {/* Plant Image */}
      <div className="flex justify-center md:-mt-40 ">
        <img src={image} alt={title} className="w-48  md:w-2xl object-contain" />
      </div>
      ...
      <div className="flex flex-col md:w-1/2 md:ml-12 ">
        ...
        <h2 className="mt-4 text-3xl md:text-4xl font-semibold">Rs. {price}/-</h2>
